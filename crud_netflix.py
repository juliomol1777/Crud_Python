...
def crear():
    while is_check_conn:
        #al ser Combobox debo activarlos pq estan solo lectura
        type_option.config(state='active')
        director_option.config(state='active')
        country_option.config(state='active')
        rating_option.config(state='active')
        duration_option.config(state='active')
        listed_in_option.config(state='active')
        #primero busco el ttulo que quiero agregar, para no agregar algo ya existente
        resultado_titulo = leer_campo(title_content.get(), 'titulo', 'title_content')
        #el seleccione... del combobox seria como un input vacio o sin datos
        if type.get() == 'Seleccione...':
            mensaje_campo_obligatorio('type')
            break
        elif director.get() == 'Seleccione...':
            mensaje_campo_obligatorio('director')
            break
        elif country.get() == 'Seleccione...':
            mensaje_campo_obligatorio('country')
            break
        elif rating.get() == 'Seleccione...':
            mensaje_campo_obligatorio('rating')
            break
        elif duration.get() == 'Seleccione...':
            mensaje_campo_obligatorio('duration')
            break
        elif listed_in.get() == 'Seleccione...':
            mensaje_campo_obligatorio('listed_in')
            break
        else:
            if resultado_titulo != []:
                #si ya existe ese titulo
